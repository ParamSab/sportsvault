            ...
            return {
                ...
                games: state.games.map(g => {
                    ...
                    if (existing >= 0) {
                        ...
                        newRsvps.push({ playerId, status, position, ...(paymentStatus !== undefined && { paymentStatus }) });
                    }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const gRes = await fetch('/api/games' + (user?.dbId ? `?userId=${user.dbId}` : ''));
                if (gRes.ok) {
                    const gData = await gRes.json();
                    if (gData.games) {
                        dispatch({ type: 'LOAD_STATE', payload: { games: gData.games } });
                    }
                }
            } catch (_) { /* ignore */ }
        }, 30000);
    ...
    useEffect(() => {
        const loadState = async () => {
            ...
            try {
                ...
                const gRes = await fetch('/api/games' + (userId ? `?userId=${userId}` : ''));
                if (gRes.ok) {
                    const gData = await gRes.json();
                    if (gData.games) {
                        dispatch({ type: 'LOAD_STATE', payload: { games: gData.games } });
