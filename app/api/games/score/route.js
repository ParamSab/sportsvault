...
export async function POST(req) {
    try {
        ...
        try {
            ...
            const { data: sbGame } = await supabase.from('Game').select('organizerId').eq('id', gameId).single();
            if (!sbGame) return Response.json({ error: 'Game not found' }, { status: 404 });
            if (sbGame.organizerId !== sessionUserId) return Response.json({ error: 'Only the organizer can set the score' }, { status: 403 });

            const { error: scoreError } = await supabase
                .from('Game')
                .update({ score: JSON.stringify(scoreData), status: 'completed' })
                .eq('id', gameId);
            if (scoreError) return Response.json({ error: scoreError.message }, { status: 500 });

            await Promise.allSettled([
                ...(team1PlayerIds || []).map(pid => updatePlayerRecordSupabase(supabase, pid, team1Result)),
                ...(team2PlayerIds || []).map(pid => updatePlayerRecordSupabase(supabase, pid, team2Result)),
            ]);
...
async function updatePlayerRecordSupabase(supabase, playerId, result) {
    const { data: user } = await supabase
        .from('User')
        .select('gamesPlayed, wins, losses, draws, trustScore')
        .eq('id', playerId)
        .maybeSingle();
    if (!user) return;
