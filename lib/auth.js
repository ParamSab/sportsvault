...
// What goes into the iron-session cookie. Browsers cap cookies at ~4KB, so
// inline base64 profile photos (hundreds of KB) must never be stored here —
// they made session.save() throw and broke every login for users with photos.
// API responses still use serializeUser (full profile incl. photo).
export function sessionUser(user) {
    const s = serializeUser(user);
    if (!s) return null;
    if (s.photo && (s.photo.startsWith('data:') || s.photo.length > 1024)) delete s.photo;
    return s;
}
