...
export function StoreProvider({ children }) {
    ...
    useEffect(() => {
        const loadState = async () => {
            ...
            // Load games, notifications, and friends in PARALLEL (was serial before).
            const userId = sessionUser?.dbId || sessionUser?.id;
            const reqs = [
                fetch('/api/games' + (userId ? `?userId=${userId}` : ''), { cache: 'no-store' }).catch(() => null),
                fetch('/api/notifications', { cache: 'no-store' }).catch(() => null),
                sessionUser ? fetch('/api/friends', { cache: 'no-store' }).catch(() => null) : Promise.resolve(null),
            ];
            const [gRes, nRes, fRes] = await Promise.all(reqs);
