...
// When iOS opens the app from a sportsvault.co.in link (Universal Link),
// Capacitor fires appUrlOpen instead of navigating the webview. Mirror the
// link's path + query into the webview so e.g. /?game=<id> lands on the game.
...
function endDateFor(game) {
  try {
    const [y, m, d] = String(game.date).split('-').map(Number);
    const [hh, mm] = String(game.time || '00:00').split(':').map(Number);
    if (!y || !m || !d) return null;
    const start = new Date(y, m - 1, d, hh || 0, mm || 0, 0, 0);
    return new Date(start.getTime() + (Number(game.duration) || 90) * 60000);
  } catch {
    return null;
  }
}
...
export async function scheduleGameReminder(game) {
  ...
  try {
    ...
    // Gentle post-game nudge ~30 min after the final whistle: rate teammates.
    const end = endDateFor(game);
    if (end) {
      const nudgeAt = new Date(end.getTime() + 30 * 60 * 1000);
      if (nudgeAt.getTime() > Date.now()) {
        notifications.push({
          id: notifIdFor(`${game.id}::rate`),
          title: `How was ${game.title || 'the game'}? 🌟`,
          body: 'Got a minute? Rate your teammates — it builds everyone’s rep.',
          schedule: { at: nudgeAt },
          extra: { gameId: game.id, kind: 'rate' },
        });
      }
    }
...
export async function cancelGameReminder(gameId) {
  ...
  try {
    ...
    await LocalNotifications.cancel({ notifications: [
      { id: notifIdFor(gameId) },
      { id: notifIdFor(`${gameId}::rate`) },
    ] });
  } catch { /* ignore */ }
}
