...
export const SPORTS = {
  // Padel is our hero/beachhead sport — listed first so it leads the sport
  // picker, filter chips, and onboarding sport selection.
...
export function getSportColor(sport) { return SPORTS[sport]?.color || '#c6f432'; }
export function getSportEmoji(sportCode) {
    if (typeof sportCode !== 'string') return '🏅';
    if (!SPORTS[sportCode]) return sportCode.slice(0, 2).toUpperCase();
    return SPORTS[sportCode].emoji;
}
...
// True once a game's scheduled slot (start + duration) is in the past. Used to
// unlock player rating right after a game ends — the DB only flips status to
// 'completed' via a periodic job (~2 days later), which is too slow for rating.
export function hasGameEnded(game) {
  if (!game || !game.date) return false;
  try {
    const start = new Date(`${game.date}T${game.time || '00:00'}`);
    if (isNaN(start.getTime())) return false;
    const end = new Date(start.getTime() + (Number(game.duration) || 0) * 60000);
...
export function formatDate(dateStr) {
  ...
  if (isNaN(d.valueOf())) return 'Recently'; // Safe guard for Invalid Date
  const now = new Date();
