...
export function StoreProvider({ children }) {
    ...
    // Polling for real-time updates (every 30 seconds) — single /api/init call
    useEffect(() => {
        if (!state.isLoaded) return;
        const interval = setInterval(async () => {
            try {
                const user = state.currentUser;
                const uid = user?.dbId || user?.id;
    ...
    // Load initial state: show localStorage instantly, then fetch fresh data in one round-trip
    useEffect(() => {
        const loadState = async () => {
            // 1. Read localStorage SYNCHRONOUSLY — show cached data instantly and mark loaded
            let cachedUser = null;
            try {
                ...
                if (saved) {
                    ...
                    if (parsed.isAuthenticated && parsed.currentUser) {
                        ...
                        // Mark isLoaded:true immediately from cache so UI renders without waiting for API
                        dispatch({ type: 'LOAD_STATE', payload: { ...parsed, games: parsed.games || [], isLoaded: true } });
                    }
                }
            } catch (_) { /* ignore */ }

            // 2. Session + all data in parallel: session (fast, no DB) + /api/init (single DB round-trip)
            const cachedUserId = cachedUser?.dbId || cachedUser?.id;
            const friendIds = (cachedUser ? [] : []).join(','); // will be populated after session if needed
            ...
            // 4. If userId changed (cache vs session), re-fetch init with real userId
            const realUserId = sessionUser?.dbId || sessionUser?.id;
            let data = null;
            try {
                if (realUserId && realUserId !== cachedUserId && initRes) {
    ...
    // Save to localStorage — only essential fields, stripped of bulk data
    useEffect(() => {
        ...
        try {
            const toSave = {
                ...
                // Slim games — strip per-player data inside rsvps to keep JSON small
