            ...
            if (user) {
                ...
                try {
                    ...
                    if (fData.friends) {
                        ...
                        dispatch({ type: 'LOAD_STATE', payload: { friends: fData.friends.map(f => f.id), players: [...fData.friends, ...PLAYERS], friendTiers: tiers } });
                    }
                } catch (_) { /* ignore */ }

                // Load games from DB and normalize to local format
                try {
                    const gRes = await fetch(`/api/games?userId=${user.dbId}`);
                    const gData = await gRes.json();
                    if (gData.games?.length > 0) {
                        const extraPlayers = [];
                        const dbGames = gData.games.map(g => {
                            const orgId = g.organizer?.id || g.organizerId;
                            // Collect organizer profile so DiscoverPage can look them up
                            if (g.organizer && orgId !== user.dbId) {
                                extraPlayers.push({
                                    id: orgId,
                                    name: g.organizer.name,
                                    photo: g.organizer.photo,
                            ...
                            return {
                                ...g,
                                // Map current user's dbId → local 'current' so all UI comparisons work
